...
export default function ConnectionRequest() {
  ...
  const handleGenerate = async () => {
    ...
    const charInstruction =
      charOption === "Auto" ? "Choose an appropriate length." : `Max ${charOption} characters.`;
      ...
      <div>
        ...
        >
          ...
          {file ? (
            ...
            <span className="text-sm text-white/40">Click to upload PDF</span>
          )}
        </button>
      ...
      {(summarizing || summaryError || profileSummary) && (
        <div className="rounded-xl border border-white/10 bg-white/5 backdrop-blur-sm p-4">
          ...
          {summarizing ? (
            ...
            <p className="text-sm text-white/70 leading-relaxed">{profileSummary}</p>
          )}
        </div>
      )}
        ...
        <>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              ...
              <input type="text" value={recipientName} onChange={(e) => setRecipientName(e.target.value)} placeholder="Jane Smith" className={fieldCls} />
            </div>
            <div>
              <label className={labelCls}>Recipient company</label>
              <input type="text" value={recipientCompany} onChange={(e) => setRecipientCompany(e.target.value)} placeholder="Acme Corp" className={fieldCls} />
            </div>
          </div>
      ...
      {message !== null && (
        <div className="rounded-xl border border-white/10 bg-white/5 backdrop-blur-sm p-5 space-y-3">
          <div className="flex items-center justify-between">
            ...
            <span className={`text-xs tabular-nums ${overLimit ? "text-red-400 font-semibold" : "text-white/30"}`}>
              {charCount}{charLimit ? ` / ${charLimit}` : ""} chars
            </span>
          </div>

          <textarea value={message} onChange={(e) => setMessage(e.target.value)} rows={6} className={fieldCls + " resize-y"} />

          <div className="flex flex-wrap gap-2 pt-1">
            <button
              onClick={handleCopy}
              className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-md text-sm font-medium border border-white/10 text-white/60 hover:bg-white/10 hover:text-white transition-colors"
            >
              Copy
            </button>
            <button
